...
class PS_Pagination {
	...
	function paginate() {
		...
		if (! $this->conn || ! is_resource($this->conn )) {
			if ($this->debug)
				echo "Database connection missing<br />";
			return false;
		}
		
		//Find total number of rows
		//$all_rs = @mysql_query($this->sql );
		$statement = oci_parse($this->conn, $this->sql);
		if(!is_resource($statement))
		{
			echo "SQL query is in wrong format<br />";
			return false;
		}
		...
		if(!$result)
		{
			echo "SQL execute error, please try again later<br />";
			return false;
		}
...
//		@mysql_close($all_rs );
		...
		$endset = $this->offset + $this->rows_per_page;
		if($endset > $this->total_rows)
			$endset = $this->total_rows;
	
		//Fetch the required result set
	//	$rs = @mysql_query($this->sql . " LIMIT {$this->offset}, {$this->rows_per_page}" );
		$newsql = "select * from (select r.*, ROWNUM as row_number from (".$this->sql.") r where ROWNUM <= $endset) where $this->offset < row_number";		
		$statement = oci_parse($this->conn, $newsql);
		if(!is_resource($statement))
		{
			echo "SQL is in wrong format!<br />";
			return false;
		}
		$result = oci_execute($statement);
		if(!$result)
